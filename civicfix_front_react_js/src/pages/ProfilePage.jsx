        ...
        <div className="d-flex flex-column min-vh-100">
            ...
            <main className="flex-grow-1 bg-light" style={{ paddingTop: '80px' }}>
                <div className="container py-5">
                    <h2 className="mb-5 text-center fw-bold" style={{ fontSize: '2.5rem' }}>Profile</h2>
                    {loading ? (
                        ...
                        <div className="row justify-content-center align-items-start">
                            ...
                            <div className="col-12 col-md-4 d-flex flex-column align-items-center mb-4 mb-md-0">
                                <img
                                    ...
                                    alt="Profile avatar"
                                    style={{ width: 160, height: 160, borderRadius: '50%', background: '#f4f4f4', border: '4px solid #D5F5E3' }}
                                />
                                <div
                                    className="mt-2"
                            ...
                            <div className="col-12 col-md-6 bg-white rounded shadow p-4 ms-md-4">
                                ...
                                {editMode && (
                                    <div className="mb-3 border rounded p-3 bg-light">
                                        ...
                                        {changePasswordMsg && (
                                            <div className={`mt-2 text-${changePasswordMsg.includes('success') ? 'success' : 'danger'}`}>
                                                {changePasswordMsg}
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="mt-5 d-flex gap-3">
                                    {editMode && (
                                        <button
                                            ...
                                            onClick={async () => {
                                                // Save Edit logic
                                                try {
                                                    const res = await fetch('/api/modify_profile', {
                                                        method: 'POST',
                                                        headers: { 'Content-Type': 'application/json' },
                                                        credentials: 'include',
                                                    ...
                                                    if (res.ok) {
                                                        setEditMode(false);
                                                        setError('');
                                                        setProfile(p => ({ ...p, username: data.username || profile.username }));
                                    ...
                                    <button className="btn btn-outline-primary px-4" onClick={() => setEditMode(v => !v)}>
                                        {editMode ? 'Cancel Edit' : 'Edit Profile'}
                                    </button>
                                    <button className="btn btn-outline-danger px-4" onClick={() => alert('Delete account feature coming soon!')}>Delete Account</button>
                                </div>
                            </div>
                        </div>
                    ) : null}
